...
import { RebalanceImpactModal } from "@/components/week/RebalanceImpactModal";
...
export function AdaptiveRebalanceBanner({
  ...
  // Estados do Modal de Impacto Antes vs. Depois
  const [isImpactModalOpen, setIsImpactModalOpen] = useState(false);
  const [impactData, setImpactData] = useState<RebalanceComparisonItem[]>([]);
  const [totalGoalHours, setTotalGoalHours] = useState(10);
  ...
  if (dismissed || !status || !status.needsRebalance) {
    return (
      <RebalanceImpactModal
        ...
        comparisons={impactData}
        totalWeeklyHours={totalGoalHours}
      />
    );
  }
  ...
  const handleApplyRebalance = () => {
    startTransition(async () => {
      ...
      if (res.success && res.comparison) {
        setImpactData(res.comparison);
        if (res.totalWeeklyHours) {
          setTotalGoalHours(res.totalWeeklyHours);
        }
        setApplied(true);

        if (onRebalanceApplied) {
          onRebalanceApplied();
        }

        // Abre o modal de impacto e recolhe o banner após breve feedback visual
        setTimeout(() => {
  ...
  return (
    <>
      ...
      {/* Modal de Impacto Antes vs Depois */}
      <RebalanceImpactModal
        isOpen={isImpactModalOpen}
        onClose={() => setIsImpactModalOpen(false)}
        comparisons={impactData}
