...
export function GoldenVersionSplitView({
      ...
      parts.push(
        <mark
          ...
          className={`px-1 py-0.5 rounded font-medium transition-colors ${
            isGolden
              ? "bg-violet-100 text-violet-800 border border-violet-200 dark:bg-violet-500/20 dark:text-violet-200 dark:border-violet-500/30"
              : "bg-indigo-100 text-indigo-800 border border-indigo-200 dark:bg-indigo-500/15 dark:text-indigo-200 dark:border-indigo-500/20"
          }`}
        >
          {matchedTerm}
        </mark>
  ...
  return (
    <div className="flex flex-col gap-5 w-full">
        ...
        <div className="flex items-center gap-3">
          <label className="flex items-center gap-2 text-xs font-semibold text-slate-700 dark:text-slate-300 cursor-pointer select-none">
            ...
            <span className="flex items-center gap-1.5 text-[11px] sm:text-xs">
              <Highlighter size={13} className="text-violet-600 dark:text-violet-400" />
          ...
          <button
            type="button"
            onClick={handleCopy}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-amber-500/10 dark:bg-amber-500/15 hover:bg-amber-500/20 dark:hover:bg-amber-500/25 text-amber-700 dark:text-amber-300 border border-amber-500/30 text-xs font-bold transition-all cursor-pointer active:scale-95 shadow-sm"
      ...
      {viewMode === "split" && (
        <div className="flex lg:hidden items-center justify-center p-1 bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-xl">
          <button
            type="button"
            onClick={() => setMobileTab("original")}
            className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition-all ${
              mobileTab === "original"
                ? "bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-white shadow"
      ...
      >
        ...
        {viewMode === "split" && (
          <div
            className={`bg-white dark:bg-white/[0.02] border border-slate-200 dark:border-white/10 rounded-2xl p-5 sm:p-6 shadow-md dark:shadow-2xl backdrop-blur-xl flex flex-col gap-4 ${
          ...
          >
            ...
            <div className="flex items-center justify-between border-b border-slate-200 dark:border-white/10 pb-3.5">
              <div className="flex items-center gap-2">
                <div className="p-1.5 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-white/5">
                  <FileText size={16} />
                </div>
                <div>
              ...
              <div className="px-3 py-1 rounded-xl bg-slate-100 dark:bg-slate-800/80 border border-slate-200 dark:border-white/10 text-xs font-mono font-bold text-slate-800 dark:text-slate-200">
                Nota: <span className="text-violet-600 dark:text-violet-400">{score.toFixed(1)}</span> / {maxScore}
              </div>
            </div>

            {/* Corpo do Texto do Aluno com Régua de Linhas */}
            <div className="bg-slate-50 dark:bg-[#090d16] border border-slate-200 dark:border-white/5 rounded-xl p-4 sm:p-5 max-h-[750px] overflow-y-auto space-y-2 select-text">
              {originalLines.map((line, idx) => (
                <div
                  key={idx}
                  className="flex gap-3 items-baseline group hover:bg-slate-200/40 dark:hover:bg-white/[0.02] rounded px-1 py-0.5 transition-colors"
                >
                  <span className="w-7 shrink-0 font-mono text-[11px] text-slate-400 dark:text-zinc-600 select-none text-right pr-2.5 border-r border-slate-200 dark:border-white/5">
                    {(idx + 1).toString().padStart(2, "0")}
                  </span>
                  <div className="flex-1 min-w-0 font-serif text-[13.5px] sm:text-[14px] leading-[26px] text-slate-800 dark:text-slate-200 break-words whitespace-pre-wrap">
            ...
            {lineErrors.length > 0 && (
              <div className="p-3 bg-slate-100/80 dark:bg-slate-950/40 border border-slate-200 dark:border-white/5 rounded-xl flex items-center justify-between text-[11px] text-slate-600 dark:text-slate-400">
        ...
        <div
          className={`bg-violet-50/60 dark:bg-violet-950/10 border border-violet-200 dark:border-violet-500/25 rounded-2xl p-5 sm:p-6 shadow-md dark:shadow-2xl backdrop-blur-xl relative overflow-hidden flex flex-col gap-4 ${
        ...
        >
          ...
          <div className="flex items-center justify-between border-b border-violet-200 dark:border-violet-500/20 pb-3.5 relative z-10">
            <div className="flex items-center gap-2">
              <div className="p-1.5 rounded-lg bg-violet-500/15 dark:bg-violet-500/20 text-violet-600 dark:text-violet-300 border border-violet-500/30">
              ...
              <div>
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[9px] font-extrabold uppercase px-1.5 py-0.5 rounded bg-amber-500/20 text-amber-700 dark:text-amber-300 border border-amber-500/30">
                    Gabaritada
                  </span>
                </div>
                <p className="text-[11px] text-slate-500 dark:text-slate-400 font-mono">
          ...
          <div className="bg-white dark:bg-[#0b0e1b] border border-violet-200 dark:border-violet-500/20 rounded-xl p-4 sm:p-5 max-h-[750px] overflow-y-auto space-y-2 relative z-10 select-text">
            {goldenLines.map((line, idx) => (
              <div
                key={idx}
                className="flex gap-3 items-baseline group hover:bg-violet-50 dark:hover:bg-violet-500/[0.04] rounded px-1 py-0.5 transition-colors"
              >
                <span className="w-7 shrink-0 font-mono text-[11px] text-violet-600/70 dark:text-violet-400/60 select-none text-right pr-2.5 border-r border-violet-200 dark:border-violet-500/15 font-semibold">
                  {(idx + 1).toString().padStart(2, "0")}
                </span>
                <div className="flex-1 min-w-0 font-serif text-[13.5px] sm:text-[14px] leading-[26px] text-slate-800 dark:text-slate-100 break-words whitespace-pre-wrap">
          ...
          <div className="bg-violet-100/60 dark:bg-slate-950/70 border border-violet-200 dark:border-violet-500/20 rounded-xl p-4 space-y-2.5 relative z-10">
            <div className="flex items-center gap-1.5 text-xs font-bold text-violet-800 dark:text-violet-300 uppercase tracking-wider">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-2 text-[11px] text-slate-700 dark:text-slate-300">
              <div className="bg-white/80 dark:bg-white/[0.02] border border-violet-200/60 dark:border-white/5 rounded-lg p-2.5 space-y-1">
              ...
              <div className="bg-white/80 dark:bg-white/[0.02] border border-violet-200/60 dark:border-white/5 rounded-lg p-2.5 space-y-1">
                <span className="font-bold text-slate-900 dark:text-white flex items-center gap-1">
                  <Scale size={12} className="text-violet-600 dark:text-violet-400 shrink-0" />
